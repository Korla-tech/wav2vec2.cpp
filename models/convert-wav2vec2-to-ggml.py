...
def convert_wav2vec2_to_ggml(model_name_or_path, output_dir, use_f16=True):
    ...
    print(f"  num_conv_pos_embeddings: {config.num_conv_pos_embeddings}")
    print(f"  num_conv_pos_embedding_groups: {config.num_conv_pos_embedding_groups}")

    # CNN feature extractor config
    conv_dim = config.conv_dim  # e.g., [512, 512, 512, 512, 512, 512, 512]
    conv_kernel = config.conv_kernel  # e.g., [10, 3, 3, 3, 3, 2, 2]
    conv_stride = config.conv_stride  # e.g., [5, 2, 2, 2, 2, 2, 2]
    n_conv_layers = len(conv_dim)
    ...
    with open(fname_out, "wb") as fout:
        # Write magic number: "wv2c" = 0x77766332
        fout.write(struct.pack("I", 0x77766332))
        ...
        fout.write(struct.pack("i", n_conv_layers))             # n_conv_layers

        # Write CNN config
        for i in range(n_conv_layers):
            fout.write(struct.pack("i", conv_dim[i]))
        for i in range(n_conv_layers):
            fout.write(struct.pack("i", conv_kernel[i]))
        for i in range(n_conv_layers):
            fout.write(struct.pack("i", conv_stride[i]))
